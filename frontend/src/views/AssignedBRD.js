...
export default function AssignedBRD(props) {
            ...
            await axiosInstance.get('/brds/assigned').then(
                result => {
                    if (result.status === 200) {
                        setCols([
                            { title: 'BRD Title', field: 'title', width: "50%" },
                            { title: 'Origin', field: 'origin', width: "30%" },
                        ...
                        if(Array.isArray(result.data) && result.data.length >= 1)
                        {
                            ...
                            pendingData.forEach((obj) => {
                                temp.push({ id: obj.id, title: obj.title, priority: obj.priority, origin: obj.origin, due_date: convertDate(obj.due_date) });
                            });
                            setRows(temp);
                        }
            ...
            <div className="brd-container content-container">
                <div className="brd-content" style={{width: "900px"}}>
                    <MaterialTable
                        ...
                        icons={tableIcons}
                        title={<h2 style={{fontFamily: "Montserrat"}}>Assigned BRDs</h2>}
                        columns={cols}
                        data={rows}  
                        actions={
                            loading === false?[{
